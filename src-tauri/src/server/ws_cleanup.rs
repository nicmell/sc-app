//! Per-WS OSC state + Phase 22 disconnect cleanup.
//!
//! Phase 29 reshape: this module used to be `session.rs` and the
//! type was `Session`. Both got renamed because Phase 29
//! introduces a different "session" concept (bridge-managed,
//! per-tab, `sessionStorage`-keyed) at a different lifetime —
//! see `server/session.rs`. This file is now narrowly the
//! per-WS-lifetime watcher that snoops `/done /notify` for the
//! clientId and fires the teardown bundle on WS close. Phase 29b
//! absorbs this watcher into the Session model when ws_bridge
//! cuts over to session-owned UDP sockets; for now they coexist.
//!
//! Owns the bits of state the bridge has to keep across datagrams:
//! - `client_id` from `/done /notify` (captured by [`WsCleanup::snoop`]
//!   on the inbound stream) — used to derive the parent group ID.
//! - `frontend_clean` flag (set by [`WsCleanup::snoop_outbound`]
//!   when it sees `/notify 0` flying outbound) — signals that the
...
/// Per-WS-lifetime cleanup watcher. Cheap to clone via
/// `Arc<WsCleanup>` so the inbound spawn task and the cleanup
/// tail can share it.
#[derive(Default)]
pub struct WsCleanup {
    state: Mutex<State>,
}
