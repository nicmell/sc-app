//! Phase 23 — global tracing subscriber init.
//!
...
//! as the process — it owns the background flush thread; dropping it
//! loses any in-flight buffered events.
//!
...
/// Called once per process: from `cli.rs::run_server_blocking` for
/// `serve` mode and from `cli.rs::run_gui` for the Tauri build.
