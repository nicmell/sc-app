...
function processElement(ctx: WalkContext): ScElementNode {
  ...
  let savedChildren: { children: ScElementNode[] } | undefined;

  if (matched && propsMatch(props, matched)) {
    ctx.element.setAttribute('id', matched.id);
    node = matched;
    savedChildren = 'children' in matched ? matched : undefined;
  } else {
  ...
  if (PARENT_TAGS.has(tag)) {
    const children = walkChildren({ element: ctx.element, matched: savedChildren, offset: 0 });
    return { ...node, children } as ScElementNode;
  }
  ...
  function visit(element: Element): ScElementNode[] {
    ...
    for (const child of Array.from(element.children)) {
      ...
      if (tag in propsExtractors) {
        result.push(processElement({ element: child, matched: ctx.matched, offset: ctx.offset }));
        ctx.offset++;
      } else {
