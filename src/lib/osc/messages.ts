...
export function bufSetnMessage(bufnum: number, start: number, values: number[]) {
  const msg = new OSC.Message(OSC_MESSAGES.BUF_SETN, bufnum, start, values.length);
  for (const v of values) msg.add(v);
  return msg;
}
